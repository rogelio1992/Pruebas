...
class ModuleHandlerTest extends KernelTestBase {
  ...
  public function testThemeMetaData() {
    ...
    $this->assertTrue(!empty($themes['bartik']->info['mtime']), 'The prueba.info.yml file modification time field is present.');
    // Use 0 if mtime isn't present, to avoid an array index notice.
    $test_mtime = !empty($themes['bartik']->info['mtime']) ? $themes['bartik']->info['mtime'] : 0;
    // Ensure the mtime field contains a number that is greater than zero.
    $this->assertTrue(is_numeric($test_mtime) && ($test_mtime > 0), 'The prueba.info.yml file modification time field contains a timestamp.');
  }
